...
public class MapGM : MonoBehaviour {
	...
	public void CreateMap(int mapx, int mapy){
		...
		int nameX = 0;
		int nameY = 0;
		map.transform.localPosition = new Vector3 ((mapx-1)*-20f*0.5f, (mapx-1)*-20f*0.5f);
		...
		for (int i = 0; i < MapRange; i++) {
			GameObject G =  Instantiate (map);
			G.SetActive (true);
			G.transform.SetParent(map.transform.parent);
			G.transform.localPosition = new Vector2 (map.transform.localPosition.x+x,map.transform.localPosition.y+ y);
